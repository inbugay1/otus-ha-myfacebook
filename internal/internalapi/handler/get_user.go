...
func (h *GetUser) Handle(responseWriter http.ResponseWriter, request *http.Request) error {
	...
	if err != nil {
		...
		return internalapi.NewServerError(fmt.Errorf("get user handler, failed to get user by id from repository: %w", err))
	}
	...
	if err != nil {
		return internalapi.NewServerError(fmt.Errorf("get user handler, cannot encode response: %w", err))
	}
